...
module.exports = async function handler(req, res) {
  ...
  if (!fullName || !email || !phone || !college || !course || !department || !game) {
  ...
  if ([fullName, college, course, department, game].some((v) => v.length > 200) || email.length > 200) {
  ...
  // Reply right away (fast ticket + green tick). The PDF email is sent in the
  // background — only if the DB-side limits allow it (see supabase_email_limit.sql).
  ...
  const mailJob = (async () => {
    try {
      ...
      if (!slot.ok) return console.error('claim_email_slot failed — did you run api/supabase_email_limit.sql?');
      if (slot.data !== true) return;
      await Promise.race([
        sendTicketEmail({
          ticketId: row.ticket_id, name: fullName, email, phone, college, course, department, game,
  ...
  try { waitUntil(mailJob); } catch { await mailJob; }
