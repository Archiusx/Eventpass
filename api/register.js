...
module.exports = async function handler(req, res) {
  ...
  const baseUrl = readEnv("db_link").replace(/\/+$/, "");
  const key = readEnv("supabase_pb_key");
  const missing = [];
  if (!baseUrl) missing.push("db_link");
  if (!key) missing.push("supabase_pb_key");
  if (missing.length) {
  ...
  if (!/^https?:\/\//i.test(baseUrl)) {
    console.error("db_link is not an http(s) URL");
    return res.status(500).json({
      error: "db_link must be the Supabase project URL (https://xxxx.supabase.co), not a Postgres connection string.",
    });
  }
