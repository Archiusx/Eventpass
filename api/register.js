...
module.exports = async function handler(req, res) {
  ...
  if (missing.length) {
    ...
    // TEMP DIAGNOSTIC (names only, never values) — remove once working.
    const seen = Object.keys(process.env).filter((k) => /supa|db_|link|anon|pb_key/i.test(k));
    return res.status(500).json({
      error:
        `Server is not configured. Missing: ${missing.join(", ")}\n` +
        `env=${process.env.VERCEL_ENV || "?"} commit=${(process.env.VERCEL_GIT_COMMIT_SHA || "?").slice(0, 7)}\n` +
        `visible env names: ${seen.length ? seen.join(", ") : "(none)"}`,
    });
  }
