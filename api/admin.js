...
// GitHub or Vercel.
//
// body: { password, action: 'stats' | 'lookup' | 'checkin' | 'undo' | 'cancel' | 'restore', ticket_id? }
...
module.exports = async function handler(req, res) {
  ...
  if (action === 'stats') {
    ...
    out = await rpc('admin_cancel', { p_password: password, p_ticket_id: id, p_cancel: action === 'cancel' });
  ...
  if (!out.ok || !out.data) {
    ...
    if (out.status === 404) {
      return res.status(502).json({ error: 'Admin setup is missing in Supabase. Run supabase_admin_upgrade.sql and supabase_analytics_upgrade.sql in the Supabase SQL Editor.' });
    }
